...
*If dùng Phương án A, sao chép file ảnh từ đường dẫn của người dùng vào `docs/tasks/[TASK-ID]/actual-ui.png`.*

### Bước 3 — So sánh trực quan giao diện

Sử dụng **task tool** của OpenCode để spawn subagent `ui-comparator` (sử dụng model `sonnet` / `oracle` để xử lý hình ảnh):
...
- Bảng danh sách lỗi chi tiết (Visual Diffs Table).
...
<!-- Gate: Xác nhận kết quả test -->
question({
  questions: [{
    question: "Xác nhận báo cáo và kết quả kiểm thử UI?",
    header: "QA Confirm",
    options: [
      { label: "Đồng ý", description: "Xác nhận kết quả, lưu báo cáo chuyển cho Developer sửa" },
      { label: "Chạy lại", description: "Tôi sẽ cung cấp lại ảnh và thực hiện test lại" }
    ]
  }]
})
